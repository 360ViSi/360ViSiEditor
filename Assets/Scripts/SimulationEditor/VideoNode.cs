...
public class VideoNode : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IDropHandler
{
    ...
    private List<ActionNode> inputNodes = new List<ActionNode>();
    ...
    public void AddInputNode(ActionNode actionNode)
    {
        inputNodes.Add(actionNode);
    }
    public void RemoveInputNode(ActionNode actionNode)
    {
        inputNodes.Remove(actionNode);
    }
    public void SetStructureManager(StructureManager manager) => structureManager = manager;
    ...
    {
        var list = getActionNodeList();
        for (int i = list.Count - 1; i > - 1; i--)
        {
            list[i].GetComponent<ActionNode>().Disconnect();
            Destroy(list[i].gameObject);
        }

        var inputNodeCount = inputNodes.Count;
        print($"Input node count: {inputNodeCount}");
        for (int i = inputNodeCount - 1; i > -1; i--)
        {
            inputNodes[i].Disconnect();
        }
